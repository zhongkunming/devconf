...
# =============================================
# Supervisor 应用管理脚本 (优化版)
# 版本: 2.0
# 路径: /home/supervisor/sbin/supervisorctl
# =============================================
...
DEFAULT_JVM_OPTS="-Xms512m -Xmx1024m -XX:+UseG1GC -XX:MaxGCPauseMillis=200 -Duser.timezone=Asia/Shanghai -Dserver.port=7088 -Dfile.encoding=UTF-8"
# 日志文件路径
...
print_help() {
    ...
    echo "日志文件位置: $LOG_DIR/supervisor.log"
...
# 功能: 清理旧备份文件
cleanup_old_backups() {
    local backup_count=$(ls -1 "$BAK_DIR" | grep "supervisor\.jar\..*$" | wc -l)
    if [ "$backup_count" -gt 10 ]; then
        echo "🧹 清理旧备份文件..."
        ls -1t "$BAK_DIR"/supervisor.jar.* | tail -n +11 | while read backup_file; do
